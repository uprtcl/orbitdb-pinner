...
const { orbitInstance, createDbInstance, createRootInstance } = require('./orbitInstance')
...
const includes = 
  async (address) => {
    const orbitdb = await orbitInstance
    const includes = await orbitdb._haveLocalData(orbitdb.cache, address);

    const key = '/'
    orbitdb.cache._store.createReadStream({
      gte: key,
